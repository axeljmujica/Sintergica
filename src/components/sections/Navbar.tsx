...
export function Navbar() {
  ...
  return (
    ...
    <>
      {isOpen && (
        <div
          className="fixed inset-0 z-40 bg-brand-midnight/5 backdrop-blur-[2px]"
          onClick={closeAll}
          aria-hidden="true"
        />
      )}
      ...
      <header className="fixed top-0 z-50 w-full">
        ...
        <div
          ...
          aria-hidden="true"
        />
        <div className={`transition-all duration-300 bg-white ${scrolled ? "border-b border-brand-midnight/[0.06]" : "border-b border-transparent"}`}>
          <div className="mx-auto flex max-w-[1400px] items-center justify-between px-6 py-3.5 lg:px-8">
                ...
                return (
                  <button
                    ...
                    className={`relative rounded-full px-4 py-2 text-[0.9rem] font-bold transition-colors duration-150 ${
                      isActive
                        ? "text-brand-midnight"
                        : "text-brand-midnight/70 hover:text-brand-midnight"
                    }`}
                  >
                    <AnimatePresence>
                      {isActive && (
                        <motion.span
                          layoutId="nav-pill"
                          className={`absolute inset-0 rounded-full border border-gray-200 bg-white shadow-sm`}
                          transition={
                            shouldReduce
                              ? { duration: 0 }
            ...
            <div className="flex items-center gap-2 lg:hidden">
              ...
              <button
                ...
                className={`rounded-lg p-2 transition-colors text-brand-midnight/60 hover:text-brand-midnight`}
              >
        ...
        {activeMenu && (() => {
            ...
            return (
              ...
              >
                <div className="flex justify-center px-6 pt-2 lg:px-8">
                  <div onMouseLeave={scheduleClose} onMouseEnter={cancelClose} className="w-full max-w-xs rounded-2xl border border-brand-midnight/[0.06] bg-brand-white p-6 shadow-xl shadow-black/5">
                    <p className="mb-4 text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-brand-midnight/40">
                    ...
                    <div className="space-y-0.5">
                      {menu.groups[0].items.map((item) => (
                        <a
                          ...
                          href={item.href}
                          onClick={closeAll}
                          className="group block rounded-lg px-3 py-2 transition-colors duration-150 hover:bg-gray-100"
                        >
                          <p className="text-[0.938rem] font-bold text-brand-midnight/90 group-hover:text-brand-midnight">
                            {item.name}
                          </p>
                        </a>
                      ))}
          ...
          /* ── Full mega menu (Lattice, Soluciones, Industrias, Investigación) ── */
          return (
            <motion.div
              key="mega-panel"
              initial={false}
            ...
            >
              <div className="flex justify-center px-6 pt-2 lg:px-8">
                <div onMouseLeave={scheduleClose} onMouseEnter={cancelClose} className={`${hasHeaderImages ? "w-full max-w-[630px]" : featuredBelow ? "w-[280px]" : hasFeatured ? "w-fit" : "w-full max-w-3xl"} rounded-2xl border border-brand-midnight/[0.06] bg-brand-white shadow-xl shadow-black/5`}>
                  {/* Soluciones: header images row */}
                  {hasHeaderImages && (
                  ...
                  <div className={hasHeaderImages ? "px-6 pb-6 pt-5" : "p-6"}>
                    <div className={`grid gap-6 ${hasFeatured && !featuredBelow ? "grid-cols-[max-content_280px]" : ""}`}>
                      ...
                      <div className={`grid gap-6 ${activeGroups.length === 2 ? "grid-cols-2" : activeGroups.length >= 3 ? "grid-cols-3" : ""}`}>
                        {activeGroups.map((group) => (
                          <div key={group.heading}>
                            <p className="mb-3 text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-brand-midnight/40">
                              {group.heading}
                            </p>
                            <div className="space-y-0.5">
                              {group.items.map((item) => (
                                <a
                                  ...
                                  href={item.href}
                                  onClick={closeAll}
                                  className="group flex items-center gap-3 rounded-lg px-3 py-2 transition-colors duration-150 hover:bg-gray-100"
                                >
                                  {item.icon && (
                                    <item.icon className="h-4.5 w-4.5 shrink-0 text-brand-midnight/40 group-hover:text-brand-midnight/70" />
                                  )}
                                  <div>
                                    <p className="text-[0.938rem] font-bold text-brand-midnight/90 group-hover:text-brand-midnight">
                                      {item.name}
                                    ...
                                    {item.desc && (
                                      <p className="mt-0.5 text-[0.8rem] leading-relaxed text-brand-midnight/50">
                                        {item.desc}
                                      </p>
                                    )}
                                  </div>
                      ...
                      {/* Featured image card — below (single column) */}
                      ...
                      {hasFeatured && !featuredBelow && menu.featured && (
                        ...
                        >
                          {menu.featured.image ? (
                            ...
                            <div className={`flex h-full flex-col justify-end bg-gradient-to-br ${menu.featured.gradient || "from-gray-100 to-gray-200"} p-6 text-brand-midnight`}>
                              <p className="text-[0.938rem] font-bold text-brand-midnight flex items-center gap-1.5">
                                {menu.featured.title}
                              </p>
                              <p className="mt-1.5 text-[0.8rem] leading-relaxed text-brand-midnight/70 font-medium">
                                {menu.featured.desc}
                              </p>
                            </div>
                          )}
                  ...
                  {/* Bottom links bar */}
                  {hasBottomLinks && (
                    <div className="flex items-center justify-end gap-10 border-t border-brand-midnight/[0.06] py-5 px-8">
                      {menu.bottomLinks!.map((link) => (
                        <a
                          key={link.label}
                          href={link.href}
                          onClick={closeAll}
                          className="inline-flex items-center gap-1.5 text-[0.6875rem] font-semibold uppercase tracking-widest text-brand-midnight/40 transition-colors hover:text-brand-midnight"
                        >
        ...
        {mobileOpen && (
          <motion.div
            ...
            className="overflow-hidden border-b border-brand-midnight/[0.06] bg-brand-white lg:hidden"
          >
          <div className="max-h-[80vh] overflow-y-auto px-5 pb-6 pt-4">
            {MENU_KEYS.map((key) => {
              ...
              return (
                <div key={key} className="border-b border-brand-midnight/[0.04] py-3 last:border-0">
                  <button
                    type="button"
                    onClick={() => setMobileExpanded(isExpanded ? null : key)}
                    className="flex w-full items-center justify-between py-1 text-sm font-semibold uppercase tracking-wider text-brand-midnight/60"
                  >
                    {menu.label}
                      ...
                      {menu.groups.map((group) => (
                        <div key={group.heading}>
                          {menu.groups.length > 1 && (
                            <p className="mb-1 mt-3 text-[0.6875rem] font-semibold uppercase tracking-wider text-brand-midnight/40">
                              {group.heading}
                            </p>
                          )}
                          {group.items.map((item) => (
                            <a
                              ...
                              href={item.href}
                              onClick={closeAll}
                              className="block rounded-lg px-2 py-2.5 text-[0.938rem] font-bold text-brand-midnight/80 transition-colors hover:bg-brand-midnight/[0.03] hover:text-brand-midnight"
                            >
                              {item.name}
                            </a>
                          ))}
